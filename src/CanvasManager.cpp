	...
	const ImVec4& paperColor)
{
	...
	Color bgColor = {
		static_cast<unsigned char>(paperColor.x * 255.0f),
		static_cast<unsigned char>(paperColor.y * 255.0f),
		static_cast<unsigned char>(paperColor.z * 255.0f),
		static_cast<unsigned char>(paperColor.w * 255.0f)
	};
	canvases.emplace_back(Canvas(width, height, fixed_name, isAnimation, useAnimTemplate, bgColor));
